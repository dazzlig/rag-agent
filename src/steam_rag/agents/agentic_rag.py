...
from steam_rag.rag_search.search_spec import EvidenceCoverageReport, SearchSpec, evaluate_evidence_coverage
...
class GameResearchAgent:
    """Investigate one game-oriented question and return evidence with a trace.

    The class used to be named ``AgenticRAGCoordinator``.  It is an agent, not
    a worker: it plans search goals, can generate HyDE evidence, decides when
    claim coverage is sufficient, and may refine the search before returning.
    The old name remains as an alias for API compatibility.
    """
    ...
    def search(
        ...
        *,
        ...
        intent = spec.intent
        plan = self._plan_from_spec(spec)[: max(1, self.config.max_steps)]
        all_results: list[SearchResult] = []
        steps: list[AgenticStep] = []
        coverage = EvidenceCoverageReport(0, 0, 0.0, ())
        ...
        for step_index, goal in enumerate(plan, start=1):
            ...
            coverage = evaluate_evidence_coverage(spec, ranked)
            sufficient = coverage.coverage_ratio >= self.config.min_claim_coverage
            steps.append(
                AgenticStep(
                    step=step_index,
                    goal=goal,
                    ...
                    claim_coverage=coverage.coverage_ratio,
                )
            )
            if sufficient:
                break
        ...
        metadata = {
            ...
            "allowed_appids": list(dict.fromkeys(int(appid) for appid in allowed_appids)),
        }
        return final_results, metadata

    def _plan_from_spec(self, spec: SearchSpec) -> list[SearchGoal]:
        goals: list[SearchGoal] = []
        for index, query in enumerate(spec.query_variants):
            claim = spec.claims[min(index, len(spec.claims) - 1)]
            goals.append(SearchGoal(claim.claim_id, query, claim.text))
        return goals
    ...
    def _is_sufficient(self, question: str, intent: str, results: Sequence[SearchResult]) -> bool:
        ...
        if self._needs_playstyle(question) and not bool({"metadata", "store_summary", "about"} & sections):
            return False
        if self._needs_updates(question) and not {"news", "analysis"} & sections:
            return False
        if self._needs_reviews(question) and "review" not in sections:
            return False
        if intent == "after_update":
            return "analysis" in sections or ("news" in sections and "review" in sections)
        if intent == "news":
...
# Backwards compatibility for the CLI, evaluation code, and older notebooks.
AgenticRAGCoordinator = GameResearchAgent
